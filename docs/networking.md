...
- Stream traffic: every type under the `stream.` prefix belongs to AetherStream watch-along sessions. `StreamSignalRouter` (src/Aetherphone/Core/Telephony/StreamSignalRouter.cs) subscribes to the same connection and dispatches the join, roster, state, queue, and kick messages to `WatchAlongSession` (src/Aetherphone/Core/Video/WatchAlongSession.cs); `CallSignalRouter` itself only suppresses its unhandled-signal warning for the prefix. When the host plays a local file, the published `Url` is an `aep-local:` fingerprint token (sampled SHA-256, byte size, and file name; src/Aetherphone/Core/Video/LocalMediaIdentity.cs) instead of a filesystem path, and each viewer maps the token to their own copy of the file before playback syncs.
