...
- Minimized mode sizes the window to the mini phone that `MinimizedPhone` measures each frame: a fixed 82 wide puck whose height grows with its vertical content (clock and date, DND moon, music or call section with a hover-expanded control row, notification card, unread badge), measured at `UiScale.Global` so it ignores the phone zoom (the window also forces the phone zoom to 1 while resting minimized). The window pins itself to a saved dock anchor that grows away from the nearest screen edge, and lerps the position between the saved maximized spot and that anchor while the morph runs. The mini phone handles its own drag (`NoMove` is set), so the anchor moves by the drag delta it reports.
