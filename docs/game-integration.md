...
| src/Aetherphone/Core/Mods/PenumbraBridge.cs | Mod list, enabled state, and window control through Penumbra's IPC |
| src/Aetherphone/Core/Mods/HeliosphereBridge.cs | Install requests to the Heliosphere plugin's local listener |
...
**Mods** follow the same bridge shape. `PenumbraBridge` (src/Aetherphone/Core/Mods/PenumbraBridge.cs) subscribes lazily to `Penumbra.ApiVersion.V5`, `Penumbra.GetModDirectory`, `Penumbra.GetModList`, `Penumbra.GetCollection`, `Penumbra.GetCurrentModSettings.V5`, `Penumbra.TrySetMod.V5`, and `Penumbra.OpenMainWindow.V5`, and listens to `Penumbra.ModAdded` and `Penumbra.ModDeleted` so the Mods app's library rescans after any install or removal. Every gate is wrapped in try/catch and reports a typed fallback; `IsAvailable` scans `InstalledPlugins` like the Lifestream bridge. The library itself is read from disk, not IPC: `ModsLibraryStore` walks `hs-*/heliosphere.json` under the Penumbra mod directory, then reads the enabled flag per mod on the framework thread. Installing never touches the mod directory from this plugin. `HeliosphereBridge` (src/Aetherphone/Core/Mods/HeliosphereBridge.cs) posts `{packageId, variantId, versionId, oneClickPassword}` to the Heliosphere plugin's listener at `http://localhost:27389/install`; with the user's one-click password Heliosphere installs silently, otherwise it opens its own confirm window, and either way its downloader, dedup, and update bookkeeping stay authoritative. A connection refusal is reported as `InstallOutcome.PluginNotRunning` and routes the user to the Setup screen. The catalog comes from Heliosphere's GraphQL endpoint through `HeliosphereApi`; see docs/plans/mods/design.md for the verified query shapes.
