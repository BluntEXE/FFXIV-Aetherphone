...
internal sealed class StreamSignalRouter : IDisposable
{
    ...
    // Speculative host-approval extension (see SignalType.StreamJoinRequest's doc comment) -
    // never raised unless the server actually sends these types, which it doesn't yet.
    public event Action<CallControl>? JoinRequested;
    public event Action<CallControl>? JoinPending;

    // Speculative shared-queue extension (see SignalType.StreamQueueSuggest's doc comment) -
    // QueueSuggested reaches the host, QueueSuggestionResult reaches the original suggester.
    // Never raised unless the server actually sends these types, which it doesn't yet.
    public event Action<CallControl>? QueueSuggested;
    public event Action<CallControl>? QueueSuggestionResult;

    // Speculative host-kick extension (see SignalType.StreamKick's doc comment) - only ever raised
    // on the client that was actually kicked. Never raised unless the server sends stream.kicked,
    ...
    // approvalRequired is the speculative host-approval extension's own field - always sent (not
    // optional) so the server has a current answer for "is this room open or gated" on every
    ...
    // Host only - decide a pending stream.joinRequest. Speculative extension: safe to call any
    // time, but only ever meaningful once the server actually sends a stream.joinRequest to
    // respond to (see WatchAlongSession.PendingRequests).
    ...
    // Viewer only - speculative shared-queue extension. suggestionId is generated by the caller
    // (WatchAlongSession.SuggestQueueItem), not the server, since a viewer can have more than one
    ...
    // Host only - decide a pending stream.queueSuggestion. Same "safe to call any time, only
    // meaningful once the server supports it" contract as Approve/Deny above.
    ...
    // Host only - force-remove a specific participant. Same "safe to call any time, only
    // meaningful once the server supports it" contract as Approve/Deny above.
