...
internal static class VolumeSlider
{
    public readonly struct Result
    {
        ...
        public Result(float value, bool dragging, bool released)
        {
            Value = value;
            Dragging = dragging;
            Released = released;
    ...
    public static Result Draw(string id, Rect row, float value, PhoneTheme theme)
    {
        var scale = ImGuiHelpers.GlobalScale;
        var drawList = ImGui.GetWindowDrawList();
        var gap = Metrics.Space.Md * scale;
        var knobRadius = KnobRadius * scale;
        var trackLeft = row.Min.X + IconColumn * scale + gap;
        var trackRight = row.Max.X - ReadoutColumn * scale - gap;
        ...
        ProgressRing.CenterIcon(drawList, new Vector2(row.Min.X + IconColumn * 0.5f * scale, midY), LevelIcon(current),
            theme.TextMuted, IconHeight * scale);
        DrawReadout(row, current, theme);
        if (hovered || dragging)
        {
